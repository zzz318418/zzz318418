...
def number_of_file():
    dir = "D:\download\canvas_screen_shoot"
    initial_count = 0
...
def makevedio():  
    ...
    videowrite = cv2.VideoWriter(r'D:\download\test.mp4',-1,30,size) # 路徑必須不包含中文 #D:\download\canvas_screen_shoot
    img_array=[]
    ...
    # 寫入影片
    for i in range(1,(quantity-3)):#for i in range(1,quantity-3)才不會IndexError: list index out of range?我也不知道為什麼:(
...
# down code is a sample use count to check pngs all ready but cant wait
"""  
# 要檢查的檔案路徑
brand_png = "D:/canvas/可用模板/good_ad/src_element/brand.png"
name_png = "D:/canvas/可用模板/good_ad/src_element/name.png"
others_png = "D:/canvas/可用模板/good_ad/src_element/others.png"
price_png = "D:/canvas/可用模板/good_ad/src_element/price.png"
prodself_png = "D:/canvas/可用模板/good_ad/src_element/prodself.png"
title_png = "D:/canvas/可用模板/good_ad/src_element/title.png"
png=[brand_png,name_png,others_png,price_png,prodself_png,title_png]
# 等待所有圖片完成生成
# 檢查檔案是否存在
count=0
for i in range(6):
    if os.path.isfile(png[i]):
        print(png[i],"存在。")
        count+=1
    else:
        print(png[i],"不存在。")
...
# down code is a sample use eel to open html in python 
"""     
eel.init('good_ad')
print('Calling Javascript...')
#eel.my_javascript_function(1, 2, 3, 4)  # This calls the Javascript function
